  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: Scaffold(
        ...
        child: customFrame(
          child: BlocProvider(
            create: (_) => audioDetailsRep..getAudioDetails(widget.id),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: [
                Padding(
                  padding: EdgeInsets.only(top:20.0),
                  child: HeaderImageAndTextWidget(
                    onPress: ()=>navigatorPop(context:context),
                    isShow: true,
                    title: 'التلاوات القرآنية',
                  ),
                ),
                BlocBuilder<AudioDetailsCubit, AudioDetailsState>(
                  builder: (_, state) {
                    if (audioDetailsRep.audioDetailsModel != null) {
                      return Column(
                        children: List.generate(
                            audioDetailsRep
                                .audioDetailsModel!.recitations!.length,
                            (index) => Padding(
                                  padding: EdgeInsets.symmetric(
                                      horizontal: 30, vertical: 15),
                                  child: InkWell(
                                    onTap: () {
                                      audioDetailsRep.initState(audioDetailsRep
                                          .audioDetailsModel!
                                          .recitations![index]
                                          .audioUrl!);
                                      if (state is PlayAudioSuccessState &&
                                          state is AudioDetailsSuccessState) {
                                      ...
                                      txt: audioDetailsRep.audioDetailsModel!
                                          ...
                                          ? (audioDetailsRep.audioDetailsModel!
                                                  ...
                                                  audioDetailsRep
                                                      .audioDetailsModel!
                                                      .recitations![1]
                                              ? recitationName[1].readersName
                                              : recitationName[2].readersName)
